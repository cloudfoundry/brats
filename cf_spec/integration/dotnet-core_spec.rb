...
describe 'For the .NET Core buildpack', language: 'dotnet-core' do
  ...
  describe 'staging with custom buildpack that uses credentials in manifest dependency uris' do
    ...
    context "using an uncached buildpack" do
      ...
      let(:dotnet_core_uri) { Regexp.new(Regexp.quote('https://-redacted-:-redacted-@buildpacks.cloudfoundry.org/dependencies/dotnet/dotnet.') + '[a-z\d\.-]+' + Regexp.quote('.linux-amd64-') + '[\da-f]+\.tar\.xz') }
    ...
    context "using a cached buildpack" do
      ...
      let(:dotnet_core_uri) { Regexp.new(Regexp.quote('https___-redacted-_-redacted-@buildpacks.cloudfoundry.org_dependencies_dotnet_dotnet.') + '[a-z\d\.-]+' + Regexp.quote('.linux-amd64-') + '[\da-f]+\.tar\.xz') }
